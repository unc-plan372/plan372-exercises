...
```{r}
total_entries = total_entries |> mutate(
  weekend = case_match(
    day_of_week,
...
The mean may cover up substantial variation. Let's make a histogram to see.

The "gg" in ggplot2 stands for "grammar of graphics." The basic idea is that we have a language
to define plots that consists of aesthetics (how aspects of the data map onto aspects of the plot)
and geoms (geometries), how those aspects are displayed. There are some other elements, e.g. scales,
which we'll cover later in the semester.

To make a histogram, we are going to map the number of entries onto the x axis, and then use a histogram
geom.
...
Some of this variation may be seasonal. Let's make a line plot showing parking demand over time.
