...
census = read_sf(file.path(DATA_PATH, "orange_durham_wake_block_groups.shp"))
```
...
```{r}
# answer
census_demographics = read_csv(file.path(DATA_PATH, "triangle_census.csv"))
```
...
census = census |>
  mutate(
    pct_african_american = race_black_african_american_alone / total_population * 100
  )
```
...
```{r}
# answer
libraries = read_csv(file.path(DATA_PATH, "Libraries.csv"))
```
...
```{r}
# answer
census$library_zone = apply(st_intersects(census, libraries_buffer), 1, any)
```
