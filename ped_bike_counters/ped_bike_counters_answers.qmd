...
```{r}
data = read_csv(here::here("ped_bike_counters", "data", "ncnmvdp.csv"), skip=2)
head(data)
```
...
A cleaner way to solve this is to just not create the hour variable until after the first summarize.
